...
from estimate_point_density import normalize_point_cloud
...
# xyz = np.asarray(normalize_point_cloud(np.asarray(pcd.points)))
...
ax = plt.axes(projection='3d')
# ax.scatter(inliers[:,0], inliers[:,1], inliers[:,2], c = 'cornflowerblue', s=0.22)
ax.scatter(outliers[:,0], outliers[:,1], outliers[:,2], c = 'salmon', s=0.22)
plt.show()